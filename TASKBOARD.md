...
| WEBPROD-029 | ⬜ | GAP MENOR: la web no maneja el evento entrante `type:"files"` → la pestaña Archivos del móvil no se refresca en vivo al generar un archivo (las imágenes sí salen inline por `token`). Cablear `mergeFiles` en el switch de `app.js` | `apps/web/app.js`, `apps/web/ui.js` | PENDIENTE (menor) |
